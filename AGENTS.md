...
- 최신 완료 지점은 PR #98이다. 작성기는 마크다운 기호를 직접 노출하지 않는 WYSIWYG 편집기로 교체해 제목·글씨 크기·굵게·기울임·밑줄·취소선을 즉시 보여준다. 데스크톱은 제목/서식/본문/첨부 도구/하단 등록 구조의 다이얼로그, 모바일은 기존 전체 화면 흐름을 유지한다. 첨부 사진은 내부 우상단의 교체·삭제 액션을 제공하며 사진이 있을 때만 하단 미리보기 설정을 표시한다. 페이지네이션 아래 중복 여백은 약 1/3 수준으로 축소했다.
