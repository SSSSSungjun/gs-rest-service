...
- 최신 완료 지점은 PR #101이다. 목록 이미지는 최대 240px에서 원본 비율로 렌더링하고, 작성기 이미지도 고정 비율·연두 배경 없이 원본 비율을 유지한다. 게시 버튼은 사진 유무와 무관하게 우측 작은 크기다. 작성 화면은 프로필·`작성자` 문구 없이 닉네임과 본문을 구분선으로 나누며, 색상 도구는 아이콘형이다. 투표는 X 삭제와 첫 선택지 자동 포커스, AI는 제목 없는 glow 패널·안내 placeholder·원형 화살표 생성 버튼을 사용한다.
