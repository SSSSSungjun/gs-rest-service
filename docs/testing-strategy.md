...
2026-07-15 GitHub-hosted Ubuntu runner와 PostgreSQL 16에서 50 VU, steady 30초를 실행했다.

- HTTP 요청 4,503건, 73.87 req/s
- check 4,503/4,503 성공
- 오류율 0%
- 응답시간 평균 3.72ms, p95 6.73ms, 최대 133.55ms
- 좋아요·투표 중복 및 고아 댓글 정합성 위반 0건

서버와 부하 발생기가 같은 runner 안에 있고 실행 시간이 짧으므로 이 값은 회귀 비교용 baseline이다. 외부 네트워크가 포함된 운영 환경의 200명 수용량을 보장하지 않는다.
## 결과 기록 형식
